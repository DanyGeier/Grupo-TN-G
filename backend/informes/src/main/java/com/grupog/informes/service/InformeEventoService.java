...
public class InformeEventoService {
    ...
    public List<EventoAgrupadoPorMes> generarInforme(FiltroEventosInput filtro) {
        ...
        if (filtro != null) {
            if (filtro.getUsuarioId() != null) builder.queryParam("usuarioId", filtro.getUsuarioId());
            if (filtro.getFechaDesde() != null) builder.queryParam("fechaDesde", filtro.getFechaDesde().toString());
            if (filtro.getFechaHasta() != null) builder.queryParam("fechaHasta", filtro.getFechaHasta().toString());
            if (filtro.getRepartoDonaciones() != null) builder.queryParam("repartoDonaciones", filtro.getRepartoDonaciones());
        }
        ...
        for (Map<String, Object> e : eventos) {
            try {
                ...
                if (nombre == null) nombre = e.get("nombre");
                ...
                if (fobj == null) fobj = e.get("fechaHora");
                if (fobj instanceof Number) {
                    fecha = LocalDateTime.ofInstant(Instant.ofEpochMilli(((Number) fobj).longValue()), ZoneId.systemDefault());
                    ...
                    try {
                        ...
                        try {
                            ...
                            fecha = LocalDateTime.ofInstant(Instant.ofEpochMilli(millis), ZoneId.systemDefault());
                        } catch (NumberFormatException ex2) {
                            try { if (s.endsWith("Z")) s = s.substring(0, s.length()-1); fecha = LocalDateTime.parse(s); } catch (Exception ignored) {}
                        }
                    }
                }
                if (fecha != null) ei.setDia(fecha.getDayOfMonth()); else ei.setDia(0);
                Object don = e.get("tieneDonacionesRepartidas");
                if (don == null) don = e.get("donaciones");
                if (don == null) don = e.get("tieneDonaciones");
                boolean tiene = false;
                if (don instanceof Boolean) tiene = (Boolean) don;
                else if (don instanceof Number) tiene = ((Number) don).intValue() != 0;
                else if (don != null) { String sd = don.toString().toLowerCase(Locale.ROOT); tiene = sd.equals("true") || sd.equals("si") || sd.equals("sí") || sd.equals("1"); }
        ...
        List<EventoAgrupadoPorMes> salida = agrupado.entrySet().stream()
            .map(ent -> { EventoAgrupadoPorMes m = new EventoAgrupadoPorMes(); m.setMes(ent.getKey()); m.setEventos(ent.getValue()); return m; })
